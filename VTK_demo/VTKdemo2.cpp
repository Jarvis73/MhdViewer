...
void showMessage(unique_ptr<myDicomImageReader> &reader, std::string &info)
{
	char buffer[2048];
	const char *ID = reader->GetPatientID();
	const char *BirthDate = reader->GetDate();
	double* origin = reader->GetOrigin();
	double* spacing = reader->GetSpacing();
	strcpy_s(PatientName, sizeof(PatientName), reader->GetPatientName());
	if (strcmp(PatientName, "?") == 0)
	{
		char *shortname = strrchr(name, 92) + 1;
...
int main(int argc, char* argv[])
{
	...
	unique_ptr<myDicomImageReader> DicomReader(new myDicomImageReader);
	if (IsMeta)
	{
	...
	{
		DicomReader->SetPath(name);
		DicomReader->init();
		DicomReader->ReadDicom();
		int length;
		length = DicomReader->GetLongestSeriesLength();
		
		//rawImage->SetNumberOfComponents(1);
		//rawImage->SetArray(data, 512 * 512 * length, 0, vtkFixedBitArray::VTK_DATA_ARRAY_DELETE);
		//rawImage->SetName("Image Data");
		
		image->SetDimensions(512, 512, length);
		image->SetOrigin(DicomReader->GetOrigin());
		image->SetSpacing(DicomReader->GetSpacing());
		image->AllocateScalars(VTK_SHORT, 1);

		Sint16 *data = (Sint16 *)image->GetScalarPointer();
		DicomReader->GetLongestSeriesData(data);

		//image->GetPointData()->SetScalars(rawImage);
	}
...
// "C:\DataSet\LungCT\2 0800412431"
// "C:\DataSet\DSB2017\sample_images\00cba091fa4ad62cc3200a657aeb957e"
